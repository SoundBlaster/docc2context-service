...
def validate_file_size(file_size: int) -> None:
    ...
    if file_size > max_size:
        ...
        raise FileValidationError(f"File size {file_size / (1024 * 1024):.1f}MB exceeds limit of {settings.max_upload_size_mb}MB")
    ...
    if not all(c in SAFE_FILENAME_CHARS for c in filename):
        ...
        logger.warning(
            ...
            extra={
                ...
                "unsafe_chars": list(unsafe_chars)
            }
        )
...
def validate_zip_bomb_protection(zip_file: zipfile.ZipFile, original_size: int) -> None:
    ...
    for file_info in zip_file.infolist():
        ...
        if file_count > max_files:
            ...
            raise FileValidationError(f"ZIP archive contains too many files: {file_count} > {max_files}")
    ...
    logger.info(
        ...
        extra={
            ...
            "file_count": file_count,
            "compression_ratio": total_uncompressed_size / original_size if original_size > 0 else 0
        }
    )
