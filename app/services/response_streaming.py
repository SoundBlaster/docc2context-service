...
class ResponseStreamer:
    ...
    async def stream_file(self, file_path: Path) -> AsyncGenerator[bytes, None]:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"file_path": str(file_path), "total_bytes": file_path.stat().st_size},
            )

        except Exception as e:
            logger.error(
                "File streaming failed", extra={"file_path": str(file_path), "error": str(e)}
            )
            raise
    ...
    async def stream_zip_on_the_fly(
        ...
        try:
            ...
            logger.info(
                ...
                extra={"total_size": total_size, "file_count": len(files_to_zip)},
            )
            ...
            logger.error(
                "ZIP streaming failed", extra={"file_count": len(files_to_zip), "error": str(e)}
            )
            raise
    ...
    def create_streaming_response(
        ...
        return StreamingResponse(
            self.stream_file(file_path), media_type=media_type, headers=headers
        )
    ...
    def create_zip_streaming_response(
        ...
        return StreamingResponse(
            self.stream_zip_on_the_fly(files_to_zip), media_type=media_type, headers=headers
        )
